...
const retrieve_values = (variable) => new Promise((resolve, reject) => {
    let res = [];
    db.serialize(() => {
        db.each(`SELECT ${variable}, AVG(age) as age, COUNT(*) as count FROM census_learn_sql GROUP BY ${variable} LIMIT 100`, (err, row) => {
            res.push(row);
        }, () => {
            db.get(`SELECT COUNT(DISTINCT ${variable}) as number FROM census_learn_sql`, (err, number) => resolve({res: res, number: number.number}))
        });
    });
});
